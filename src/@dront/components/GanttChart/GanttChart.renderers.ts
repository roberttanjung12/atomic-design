...
export const renderTaskItem =
  (data: Task[]) => (params: CustomSeriesRenderItemParams, api: CustomSeriesRenderItemAPI) => {
    ...
    return { type: 'group', children: [mainBar, progressOverlay] } as echarts.CustomSeriesRenderItemReturn;
  };
...
export const renderDependencyItem =
  (data: Task[]) => (params: CustomSeriesRenderItemParams, api: CustomSeriesRenderItemAPI) => {
    ...
    const coordSys = params.coordSys as any;
    const verticalRunX = Math.min(P1_X, P4_X) - HORIZONTAL_OFFSET;
    ...
    return {
      ...
      clipPath: {
        type: 'rect',
        shape: { x: coordSys.x - 100, y: coordSys.y, width: coordSys.width + 100, height: coordSys.height }
      },
      children: [
        {
          type: 'path',
